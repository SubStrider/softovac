...
(function($) {
    ...
    $.fn.simpleGauge = function(options) {
        ...
        $(this).each(function() {
            ...
            if (value) {
                $(this).find('.gauge-wrap-before').css('background', 'hsla(' + Math.round(lowColor) + ', ' + settings.saturation + ', ' + settings.lightness + ', 1)');
                $(this).find('.gauge-active').css('background', 'linear-gradient(90deg, hsla(' + Math.round(activeColor) + ', ' + settings.saturation + ', ' + settings.lightness + ', 1),hsla(' + Math.round(lowColor) + ', ' + settings.saturation + ', ' + settings.lightness + ', 1))');
                var angle = (150 * (value / 100)) + 100;
                $(this).find('.indicator').css('transform', 'rotate(' + angle + 'deg)');
            } else {
